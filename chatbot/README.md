...
|---|---|
| **Prompt formatting** — how a system prompt + conversation turns become one API request | [`chat_engine.py`](chat_engine.py) → `format_messages()` |
| **Conversation history** — how state persists across Streamlit reruns | [`app.py`](app.py) → `st.session_state.messages` |
| **App structure** — clean separation of UI, state, and API logic | `app.py` handles UI; `chat_engine.py` handles the model |
